...
def singleblog(event):
    print(event)
    cursor.execute("SELECT event_name, club_name, service, email, phone, details from eventsdetails WHERE event_name='"+event+"';")
    data = cursor.fetchone()
    print(data)
...
#         fcm_token = content['fcm_token']
        cursor.execute("SELECT * FROM userdetail WHERE email ='" + email + "' and  password = '" + password + "'")
        re = cursor.fetchone()
